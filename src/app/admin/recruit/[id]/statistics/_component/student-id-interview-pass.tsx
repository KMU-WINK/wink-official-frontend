...
interface StudentIdInterviewPassProps {
  forms: RecruitForm[];
}

export default function StudentIdInterviewPassChart({ forms }: StudentIdInterviewPassProps) {
  const data = useMemo(() => {
    return forms
  ...
  const length = useMemo(
    () => forms.filter((x) => x.paperPass).filter((x) => x.interviewPass).length,
    [forms],
  );
