...
interface StudentIdPaperTotalProps {
  forms: RecruitForm[];
}

export default function StudentIdPaperTotalChart({ forms }: StudentIdPaperTotalProps) {
  const data = useMemo(() => {
    return forms.reduce<DataType[]>((acc, form) => {
  ...
  const chartConfig = useMemo<ChartConfig>(() => {
    return forms.reduce<ChartConfig>((acc, form) => {
  ...
  const length = useMemo(() => forms.length, [forms]);
