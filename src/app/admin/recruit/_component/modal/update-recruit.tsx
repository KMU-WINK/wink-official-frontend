...
import { cn, formatDate, formatDateApi, toDate } from '@/util';
...
export default function UpdateRecruitModal({
  open,
  setOpen,
  recruit,
  callback,
}: CreateRecruitModalProps) {
  ...
  const onSubmit = useCallback(
    async (values: CreateRecruitRequest) => {
      setIsProcessing(true);

      const { recruit: _recruit } = await Api.Domain.AdminRecruit.updateRecruit(
        recruit!.id,
        values,
      );
  ...
  useEffect(() => {
    ...
    setRecruitDate({
      from: subHours(toDate(recruit.recruitStartDate), 9),
      to: subHours(toDate(recruit.recruitEndDate), 9),
    });

    setInterviewDate({
      from: subHours(toDate(recruit.interviewStartDate), 9),
      to: subHours(toDate(recruit.interviewEndDate), 9),
    });
  }, [recruit]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>모집 수정</DialogTitle>
          <DialogDescription>모집을 수정합니다.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
